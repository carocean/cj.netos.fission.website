...
public class WXSignWebview extends WXAuthUtil implements IGatewayAppSiteWayWebView {
    ...
    public void flow(Frame frame, Circuit circuit, IGatewayAppSiteResource resource) throws CircuitException {
        ...
        CJSystem.logging().info(getClass(),"accessToken:" + accessToken + "\njsapi_ticket:" + jsapi_ticket + "\n时间戳：" + timestamp + "\n随机字符串：" + noncestr);
...
//        String url = String.format("http://%s/%s/home.html?state=%s", frame.head("Host"), frame.rootName(),frame.parameter("state"));
        //在生产nginx使用80端口，前端无80，而后台有80，这样会导致失败，干脆写死域名
        String url = String.format("http://nodespower.com/%s/home.html?state=%s",  frame.rootName(),frame.parameter("state"));
        ...
        CJSystem.logging().info(getClass(),"参数：" + str + "\n签名：" + signature);
